...
void AnsList::addDoneJob( int OID, int arrival, int duration, int cpuTime ) { // show the job list 
	doneType job;
	job.OID = OID, job.departure = cpuTime, job.delay = cpuTime - arrival;
...
void Simulation::SQF() { // shortest queue first
	...
	while( !jobs.isEmpty() && !allQ[i]->isEmpty() ) {
		...
		if( times == allQ[i]->avail + Process.duration ) {
			allQ[i]->avail = times;
			answer.addDoneJob( Process.OID, Process.arrival, Process.duration, times );
			allQ[i]->deQueue(Process); 
		}
		
		if( times >= Process.timeout  ) {
			answer.addAbortJob( Process.OID, Process.arrival, Process.timeout, times );
			allQ[i]->avail = times;
			if( !allQ[i]->isEmpty()) allQ[i]->deQueue(Process);
